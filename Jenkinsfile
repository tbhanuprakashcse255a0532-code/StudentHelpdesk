  ...
  stages {
    stage('Plan') {
      steps {
        checkout scm
        script {
          if (!fileExists('docs/PLAN.md')) { error('Missing project plan') }
          if (isUnix()) { sh 'node scripts/check-runtime.cjs' }
          else { bat 'node scripts/check-runtime.cjs' }
        }
      }
    }
    stage('Code') {
      steps {
        script {
          if (isUnix()) { sh 'node scripts/check-repository.cjs' }
          else { bat 'node scripts/check-repository.cjs' }
        }
      }
    }
    stage('Build') {
      steps {
        script {
          if (isUnix()) {
            ...
            sh 'npm --prefix frontend test -- --watchAll=false'
